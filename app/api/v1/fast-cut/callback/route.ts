...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Fast Cut Callback] Received: job_id=${job_id}, status=${status}`);
    ...
    if (!generation) {
      console.error(`[Fast Cut Callback] Generation not found: ${job_id}`);
      return NextResponse.json({ detail: 'Generation not found' }, { status: 404 });
    }

    // Skip if already completed/failed (idempotency)
    if (generation.status === 'COMPLETED' || generation.status === 'FAILED') {
      console.log(`[Fast Cut Callback] Already ${generation.status}, skipping: ${job_id}`);
    ...
    if (status === 'completed' && output_url) {
      ...
      console.log(`[Fast Cut Callback] Updated to COMPLETED: ${job_id}`);
      ...
      if (autoPublish?.enabled) {
        try {
          ...
          }).catch(err => console.error('[Fast Cut Callback] Auto-schedule failed:', err));
        } catch (scheduleError) {
          console.error('[Fast Cut Callback] Auto-schedule error:', scheduleError);
        }
      }
      ...
      console.log(`[Fast Cut Callback] Updated to FAILED: ${job_id}, error: ${error}`);
    }
    ...
    console.error('[Fast Cut Callback] Error:', err);
    return NextResponse.json(
